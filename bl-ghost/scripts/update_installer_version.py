...
# WIN INSTALLER
  ...
  for line in fileinput.input(projectpath + "/installer/" + config['BUNDLE_NAME'] + ".iss",inplace=1):
    ...
    if "OutputBaseFilename" in line:
      if demo:
        line="OutputBaseFilename=bl-ghost-v" + config['FULL_VER_STR'] + "[DEMO]-Installer\n"
      else:
        line="OutputBaseFilename=bl-ghost-v" + config['FULL_VER_STR'] + "-Installer\n"
    ...
    if "WelcomeLabel1" in line:
     if demo:
       line="WelcomeLabel1=Welcome to the bl-ghost Demo installer\n"
     else:
       line="WelcomeLabel1=Welcome to the bl-ghost installer\n"
       
    if "SetupWindowTitle" in line:
     if demo:
       line="SetupWindowTitle=bl-ghost [DEMO] installer\n"
     else:
       line="SetupWindowTitle=bl-ghost installer\n"
