...
make -k app BUILD_CONFIG=Release CMAKE_CXX_FLAGS="-DDEMO_VERSION=$DEMO"
...
cp -R build-linux/$PLUGIN_NAME-app installer/$ZIP_NAME

#cp -R manual/${PLUGIN_NAME}_manual-EN.pdf installer/$ZIP_NAME
cp -R manual/bl-ghost-X_manual-EN.pdf installer/$ZIP_NAME
cp -R installer/license.rtf installer/$ZIP_NAME
cp -R installer/install-plug.sh installer/$ZIP_NAME
cp -R installer/install-app.sh installer/$ZIP_NAME
...
strip installer/$ZIP_NAME/$PLUGIN_NAME-app/$PLUGIN_NAME
