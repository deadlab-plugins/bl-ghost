...
set PT_GUID="487D2070-64DD-11E8-8B50-005056A204F3"

if %1 == 1 (echo Making bl-ghost Windows DEMO VERSION distribution ...) else (echo Making bl-ghost Windows FULL VERSION distribution ...)
...
REM - msbuild bl-ghost-app.vcxproj /p:configuration=release /p:platform=win32

echo Building 32 bit binaries...
msbuild bl-ghost.sln /p:configuration=release /p:platform=win32 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly 

echo Building 64 bit binaries...
msbuild bl-ghost.sln /p:configuration=release /p:platform=x64 /nologo /verbosity:minimal /fileLogger /m /flp:logfile=build-win.log;errorsonly;append
...
REM move .\build-win\aax\Win32\Release\bl-ghost.aaxplugin .\build-win\aax\Win32\Release\bl-ghost.aaxplugin_unsigned

REM %WRAPTOOL% sign --verbose --account nicolas.suede --password "Shells222" --keyfile "..\..\..\Certificate\BlueLab.p12" --keypassword "FY@Somewh3re" --wcguid %PT_GUID% --in .\build-win\aax\Win32\Release\bl-ghost.aaxplugin_unsigned --out .\build-win\aax\Win32\Release\bl-ghost.aaxplugin

REM move .\build-win\aax\x64\Release\bl-ghost.aaxplugin .\build-win\aax\x64\Release\bl-ghost.aaxplugin_unsigned

REM %WRAPTOOL% sign --verbose --account nicolas.suede --password "Shells222" --keyfile "..\..\..\Certificate\BlueLab.p12" --keypassword "FY@Somewh3re" --wcguid %PT_GUID% --in .\build-win\aax\x64\Release\bl-ghost.aaxplugin_unsigned --out .\build-win\aax\x64\Release\bl-ghost.aaxplugin
...
REM /cc
:32-Bit-is
"%ProgramFiles%\Inno Setup 5\iscc" /Q ".\installer\bl-ghost.iss"
goto END-is

REM /cc
:64-Bit-is
"%ProgramFiles(x86)%\Inno Setup 5\iscc" /Q ".\installer\bl-ghost.iss"
goto END-is

:END-is

REM - Codesign Installer for Windows 8+
REM -"C:\Program Files (x86)\Microsoft SDKs\Windows\v7.1A\Bin\signtool.exe" sign /f "XXXXX.p12" /p XXXXX /d "bl-ghost Installer" ".\installer\bl-ghost Installer.exe"

REM -if %1 == 1 (
REM -copy ".\installer\bl-ghost Installer.exe" ".\installer\bl-ghost Demo Installer.exe"
REM -del ".\installer\bl-ghost Installer.exe"
REM -)
